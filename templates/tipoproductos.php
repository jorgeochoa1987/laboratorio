...
<div class="app-main__inner">
                        <div class="app-page-title">
                            <div class="page-title-wrapper">
                                <div class="page-title-heading">
                                    ...
                                    <div>Listado Tipo de Productos
                                        <div class="page-title-subheading">Actualmente cuenta con # de tipos de productos registrados</div>
                                    </div>
                                </div>
               ...
               <body>
               <?php  
               $usuario = "SELECT `id`, `nombre`, `descripcion` FROM `tipoProductos`" ;
               ...
               //echo $usuario;
                while ($row = mysqli_fetch_assoc( $resultado))  {?>                             
                  ...
                  <td> <button  class="btn btn-info"   data-toggle="modal" data-target="#modaleditar" onclick="subireditartipoproductos(<?php echo $row['id'] ?>)"> Editar </button> 
                  <button   class="btn btn-danger"  onclick="eliminartipoproductos(<?php echo $row['id'];?>)">Eliminar </button>
                  </td> 
                  </tr> 
               <?php }?> 
                  <td class="nuevos"></td>
    ...
    <div class="modal-content">
      ...
      <form class="">
                    <div class="position-relative form-group"><label for="nombre" class="">Nombre</label>
                        <input id="txt-nombretipoproductos" type="text" placeholder="nombre" required class="form-control"></div>
                    <div class="position-relative form-group"><label for="descripcion" class="">Descripción</label>
                        <input id="txt-descripciontipoproductos" type="text" placeholder="Descripcion" required class="form-control"></div>
                    
         </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" id="btn-cerrartipoproductos" data-dismiss="modal">Cerrar</button>
        <button class="mt-1 btn btn-primary" type="submit" id="btn-guardartipoproductos" value="Guardar">Guardar</button>
      </div>
    </div>
  </div>
</div> 
<!--Modal editar-->
 ...
 <div class="modal fade" id="modaleditar" tabindex="-1" role="dialog" aria-labelledby="labeleditar" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="labeleditar">Editar Tipo Producto</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
